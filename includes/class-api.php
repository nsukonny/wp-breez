...
class API
{
    ...
    private string $api_url = 'https://api.breez.ru/v1/';
    ...
    public function get_breez_products_stocks(): array
    {
        ...
        if (empty($breez_products_stocks)) {
    ...
    private function get_response($url): array
    {
        ...
        if (empty($settings['api_key']) || empty($settings['username']) || empty($settings['passwd'])) {
            return [];
        }

        $credentials = base64_encode($settings['username'] . ":" . $settings['passwd']);
        $args = array(
            'headers' => array(
